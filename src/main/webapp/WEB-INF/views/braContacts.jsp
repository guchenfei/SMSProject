...
<body>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title">修改本公司联系人</h4>
				</div>
				<div class="modal-body">
					<form class="form-horizontal">
						<div class="form-group">
			...
			<div class="col-md-3">
				<ul class="toolbar">
					<li id="contacts_add_modal_btn">
						...
						<font style="font-size: 11px">添加本公司联系人</font>
					</li>
				</ul>
			</div>
			<div class="col-md-3">
	...
	<script type="text/javascript">
		...
		$(function() {
			//去首页
			to_page(currentCompany, 1);
		});

		//发送指定的请求，跳到点击的页码页
		function to_page(currentCompany, pn) {
			$.ajax({
		...
		function build_page_nav(result) {
			...
			if (result.extend.pageInfo.hasNextPage == false) {
				...
				lastPageLi.click(function() {
					to_page(currentCompany, result.extend.pageInfo.pages);
				});
			}
			//添加首页和前一页的提示
										...
										success : function(result) {
											if (result.code == 100) {
												...
												$("#contactsAddModal").modal(
														'hide');
												//2，来到最后一页，显示刚才保存的数据
												to_page(currentCompany, totalRecord);
											...
											success : function(result) {
												...
												if (result.code == 100) {
													...
													$("#contactsUpdateModal")
															.modal('hide');
													//2,回到本页面
													to_page(currentCompany, currentPage);
												} else if (result.code == 200) {
													//显示失败信息之前需要清除以前的样式
